...
SET filename=SolARSample_SLAM_%version%
SET arg1=%1
...
remaken install -v packagedependencies.txt || (echo "Error: could not remaken install release deps" && EXIT /B 1)
remaken install -v packagedependencies.txt -c debug || (echo "Error: could not remaken install debug deps" && EXIT /B 1)
